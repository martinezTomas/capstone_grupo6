...
from .forms import RegistroUserForm, ProductoForm, UserUpdateForm, EditarUsuarioAdminForm, ResenaForm, MascotaForm
...
def index(request):
    ...
    ofertas = Producto.objects.filter(visible=True, en_oferta=True).order_by('?')[:4]
    
    ids_ofertas = list(ofertas.values_list('id', flat=True))
    ...
    if top_producto_ids:
        ...
        productos_para_mostrar = Producto.objects.filter(
            pk__in=top_producto_ids, 
            visible=True
        ).exclude(id__in=ids_ofertas).order_by(preserved_order)[:4] 
        
    else:
        productos_para_mostrar = Producto.objects.filter(
            visible=True
        ).exclude(id__in=ids_ofertas).order_by('-id')[:4]
...
def catalogo_view(request, solo_ofertas=False):
    ...
    if solo_ofertas:
        ...
        productos = Producto.objects.filter(visible=True).order_by('-en_oferta', 'nombre')
...
def get_razas_por_especie(request):
    """
    API para obtener las razas filtradas por especie vía AJAX.
    """
    ...
    if especie_id:
        try:
            # Filtramos las razas que pertenecen a esa especie
            razas = list(Raza.objects.filter(especie_id=especie_id).values('id', 'nombre').order_by('nombre'))
        except Exception as e:
            print(f"Error buscando razas: {e}")
...
def editar_usuario_admin(request, pk):
    ...
    if request.method == 'POST':
        form = EditarUsuarioAdminForm(request.POST, instance=usuario_editar)
        if form.is_valid():
            form.save()
            messages.success(request, f'El usuario {usuario_editar.username} ha sido actualizado correctamente.')
            return redirect('gestion_usuarios')
    else:
        form = EditarUsuarioAdminForm(instance=usuario_editar)

    context = {
        'form': form,
        'usuario_editar': usuario_editar
    }
    return render(request, 'paneladmin/gestion_usuario/editar_usuario.html', context)
