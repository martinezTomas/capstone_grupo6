...
from .models import Producto, Categoria, Subcategoria, Especie, Marca, Resena, Mascota, Especie, Raza, Condicion, Edad
...
class EditarUsuarioAdminForm(forms.ModelForm):
    class Meta:
        ...
        labels = {
            ...
            'is_active': 'Cuenta Activa (No baneado)',
        }
        help_texts = {
            'is_staff': 'Marca esta casilla si quieres que este usuario tenga acceso al Panel de Admin.',
            'is_active': 'Desmarca esta casilla para bloquear el acceso a este usuario sin borrarlo.',
        }
...
class ProductoForm(forms.ModelForm):
    class Meta:
        ...
        labels = {
            ...
            'en_oferta': '¿Oferta Manual? (Opcional)',
            'edades': 'Etapas de Vida (Seleccione varias)',
            'condiciones': 'Condiciones Médicas (Seleccione varias)',
            'razas': 'Razas Específicas (Opcional)',
        }
        ...
        widgets = {
            ...
            # --- WIDGETS NUEVOS PARA CAMPOS M2M y BOOLEANOS ---
            'visible': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            'en_oferta': forms.CheckboxInput(attrs={'class': 'form-check-input'}),
            
            'edades': forms.CheckboxSelectMultiple(),
            'condiciones': forms.CheckboxSelectMultiple(),
            
            # SelectMultiple es mejor para razas porque son muchas
            'razas': forms.SelectMultiple(attrs={'class': 'form-select', 'style': 'height: 150px;'}),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Ordenamos desplegables básicos
        ...
        # Ordenamos los nuevos campos ManyToMany
        self.fields['edades'].queryset = Edad.objects.all().order_by('nombre')
        self.fields['condiciones'].queryset = Condicion.objects.all().order_by('nombre')
        self.fields['razas'].queryset = Raza.objects.all().order_by('nombre')
